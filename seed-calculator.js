...
async function seedCalculator() {
  try {
    ...
    // Clear existing calculator data
    await Calculator.deleteMany({});
    console.log('🗑️  Cleared existing calculator data');
    ...
    const calculator = await Calculator.create(calculatorData);
    console.log('✅ Calculator seeded successfully!');
    console.log(`📊 Created calculator with ${calculator.steps.length} steps`);
    console.log(`📝 Title: ${calculator.title}`);
    console.log(`💰 Base Price: ₹${calculator.basePrice.toLocaleString()}`);
